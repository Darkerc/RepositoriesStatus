...
/**
 * Colores base por defecto para cada modo de visualización (nivel 4, el más oscuro).
...
/**
 * Escalas de colores por defecto para cada modo de visualización.
...
/**
 * Escalas de colores activas (pueden ser personalizadas por el usuario).
 * @type {Object.<string, string[]>}
 */
let COLORS = { ...DEFAULT_COLORS };

/** @type {{github: string, gitlab: string, mixed: string}|null} Colores base personalizados activos */
let customBaseColors = null;

/**
 * Convierte un color hexadecimal a componentes HSL.
 * @param {string} hex - Color en formato '#RRGGBB'.
 * @returns {{h: number, s: number, l: number}} Valores HSL (h: 0-360, s/l: 0-100).
 */
...
/**
 * Convierte valores HSL a un color hexadecimal.
...
/**
 * Genera una escala de 5 niveles de color a partir de un color base.
 * El nivel 0 es un placeholder (se reemplaza en render por el color vacío del tema).
 * Los niveles 1-4 interpolan la luminosidad desde un valor claro hasta el color base.
 *
 * @param {string} baseHex - Color más oscuro (nivel 4) en formato '#RRGGBB'.
 * @returns {string[]} Array de 5 colores hex [placeholder, nivel1, nivel2, nivel3, nivel4].
 */
function generateScale(baseHex) {
  ...
  for (let i = 1; i <= steps; i++) {
    const lightness = lightMax - ((lightMax - l) * i) / steps;
    scale.push(hslToHex(h, s, Math.round(lightness)));
  }
  return scale;
}

/**
 * Aplica colores base personalizados. Pasar null restaura los valores por defecto.
 * @param {{github: string, gitlab: string, mixed: string}|null} colors - Colores base o null.
 */
export function setCustomColors(colors) {
  customBaseColors = colors;
  if (colors) {
    COLORS = {
      github: generateScale(colors.github),
      gitlab: generateScale(colors.gitlab),
      mixed:  generateScale(colors.mixed),
    };
  } else {
...
/**
 * Retorna los colores base actuales (personalizados o por defecto).
 * @returns {{github: string, gitlab: string, mixed: string}}
 */
export function getBaseColors() {
  return customBaseColors || { ...DEFAULT_BASE_COLORS };
}
...
function renderLegend(container, githubData, gitlabData) {
  ...
  // Estadísticas de días trabajados (total + por plataforma conectada),
  // a la izquierda de la clave de colores por proveedor.
  if (hasGitHub || hasGitLab) {
    html += '<span class="legend-divider"></span>';
    const total = countTotalWorkedDays(githubData, gitlabData);
    html += `<span class="stats-item stats-total">` +
      `<span class="stats-label">${t('heatmap.daysWorkedTotal')}</span>` +
      `<span class="stats-value">${total}</span>` +
    `</span>`;
    if (hasGitHub) {
      html += `<span class="stats-item stats-github">` +
        `<span class="provider-dot gh-dot"></span>` +
        `<span class="stats-label">${t('heatmap.daysWorkedGitHub')}</span>` +
        `<span class="stats-value">${countWorkedDays(githubData)}</span>` +
      `</span>`;
    }
    if (hasGitLab) {
      html += `<span class="stats-item stats-gitlab">` +
        `<span class="provider-dot gl-dot"></span>` +
        `<span class="stats-label">${t('heatmap.daysWorkedGitLab')}</span>` +
        `<span class="stats-value">${countWorkedDays(gitlabData)}</span>` +
      `</span>`;
    }
  }
...
/**
 * Cuenta los días con al menos una contribución en el mapa dado.
 *
 * @param {Object.<string, number>} data - Mapa fecha → conteo de contribuciones.
 * @returns {number}
 */
function countWorkedDays(data) {
  let n = 0;
  for (const k in data) if (data[k] > 0) n++;
  return n;
}

/**
 * Cuenta los días únicos con actividad en cualquiera de las dos plataformas
 * (unión de fechas: un día con actividad en ambas plataformas cuenta una sola vez).
 *
 * @param {Object.<string, number>} github
 * @param {Object.<string, number>} gitlab
 * @returns {number}
 */
function countTotalWorkedDays(github, gitlab) {
  const set = new Set();
  for (const k in github) if (github[k] > 0) set.add(k);
  for (const k in gitlab) if (gitlab[k] > 0) set.add(k);
  return set.size;
}
