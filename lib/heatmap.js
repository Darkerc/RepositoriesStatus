 ...
 *
 * Lógica de selección:
 * - Si no hay contribuciones (nivel 0), siempre usa el color gris base.
...
function getCellColor(githubCount, gitlabCount) {
  ...
  // Sin contribuciones: siempre color gris base
...
function buildTooltip(dateStr, githubCount, gitlabCount) {
  // Parsear la fecha y formatearla de forma legible
  ...
  // Generar texto según qué proveedores tienen contribuciones
  if (githubCount > 0 && gitlabCount > 0) {
...
export function renderHeatmap(container, githubData, gitlabData) {
  ...
  const dayLabels = [
    { label: 'Mon', row: 1 },  // Lunes es fila 1 (Domingo es 0)
    { label: 'Wed', row: 3 },  // Miércoles
    { label: 'Fri', row: 5 },  // Viernes
  ];
