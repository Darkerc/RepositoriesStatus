...
/**
 * Concurrencia máxima al pedir /languages por proyecto. Mantiene el throughput
 * sin disparar el rate limiting de GitLab.
 * @const {number}
 */
const LANG_FETCH_CONCURRENCY = 6;

/**
 * Ejecuta una lista de promise factories con un máximo de concurrencia.
 * Cada item de `tasks` es una función que devuelve una Promise. Se inician
 * `limit` promises a la vez y, conforme van resolviendo, se inician las siguientes.
 *
 * @template T
 * @param {Array<() => Promise<T>>} tasks - Factories de promesas a ejecutar.
 * @param {number} limit - Concurrencia máxima.
 * @returns {Promise<Array<T>>} Resultados en el mismo orden que `tasks`.
 */
...
/**
 * Obtiene un mapa agregado de lenguajes más usados del usuario en GitLab.
 *
 * Estrategia (porque GitLab solo expone porcentajes por proyecto, no bytes):
 * 1. Lista los 30 proyectos más activos del usuario (membership=true) con
 *    `statistics=true` para conocer el tamaño en bytes de cada repo.
 * 2. Por cada proyecto, pide /languages que devuelve { nombre: porcentaje }.
 * 3. Agrega bytes_lenguaje = (porcentaje / 100) * repository_size, sumando
 *    a través de todos los proyectos.
 *
 * GitLab no provee colores oficiales por lenguaje, por lo que `color` se deja
 * en `null`; el renderer aplicará un mapa de colores comunes y, en última
 * instancia, un color derivado del nombre.
 *
 * @returns {Promise<{languages: Object.<string, {bytes: number, color: null}>, repoCount: number}>}
 * @throws {Error} 'Not authenticated with GitLab' si no hay token.
 * @throws {Error} 'UNAUTHORIZED' si el token es inválido (HTTP 401).
 */
export async function fetchGitLabLanguages() {
  const token = await getToken('gitlab');
  if (!token) throw new Error('Not authenticated with GitLab');

  const baseUrl = await getGitLabBaseUrl();

  // Listar los 30 proyectos del usuario más recientemente activos. statistics=true
  // expone repository_size (bytes) que necesitamos para ponderar los porcentajes.
  const projResp = await fetch(
    `${baseUrl}/api/v4/projects?membership=true&order_by=last_activity_at&sort=desc&per_page=30&statistics=true`,
    { headers: { 'Authorization': `Bearer ${token}` } }
  );
  if (projResp.status === 401) throw new Error('UNAUTHORIZED');
  if (!projResp.ok) {
    if (projResp.status === 429) throw new Error('RATE_LIMITED');
    throw new Error(`GitLab projects API error: ${projResp.status}`);
  }
  ...
  // Pedir /languages por proyecto con concurrencia limitada.
  const tasks = projects.map(p => async () => {
    const resp = await fetch(`${baseUrl}/api/v4/projects/${p.id}/languages`, {
      headers: { 'Authorization': `Bearer ${token}` },
    });
    if (resp.status === 401) throw new Error('UNAUTHORIZED');
    if (!resp.ok) return null;
    const langs = await resp.json();
    return { project: p, langs };
  });

  let perProject;
  try {
    perProject = await runWithConcurrency(tasks, LANG_FETCH_CONCURRENCY);
  } catch (err) {
    if (err.message === 'UNAUTHORIZED') throw err;
    perProject = [];
  }

  // Agregar bytes ponderados por repository_size.
  const languages = {};
  let repoCount = 0;
  for (const result of perProject) {
    if (!result || !result.langs) continue;
    const repoSize = result.project?.statistics?.repository_size || 0;
    // Si no tenemos bytes (proyecto sin statistics o vacío), omitir para no sesgar.
    if (!repoSize) continue;
    repoCount++;
    for (const [name, percent] of Object.entries(result.langs)) {
      const pct = Number(percent);
      if (!Number.isFinite(pct) || pct <= 0) continue;
      if (!languages[name]) {
        languages[name] = { bytes: 0, color: null };
      }
      languages[name].bytes += (pct / 100) * repoSize;
    }
  }

  return { languages, repoCount };
}
