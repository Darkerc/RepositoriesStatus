 ...
 * @throws {Error} 'UNAUTHORIZED' si el token es inválido.
 */
export async function fetchGitLabGroupActivity(groupId) {
  const token = await getToken('gitlab');
  if (!token) throw new Error('Not authenticated with GitLab');

  const baseUrl = await getGitLabBaseUrl();

  // Paso 1: top 8 proyectos del grupo por actividad reciente
  const projResp = await fetch(
    `${baseUrl}/api/v4/groups/${groupId}/projects?order_by=last_activity_at&per_page=8&simple=true&include_subgroups=true`,
    { headers: { 'Authorization': `Bearer ${token}` } }
  );
  ...
  // Paso 2: construir projectMap a partir de la respuesta (evita fetchs adicionales)
  const projectMap = {};
  for (const p of projects) {
    projectMap[p.id] = p.path_with_namespace || `project/${p.id}`;
  }

  // Paso 3: fetch paralelo de eventos por proyecto (10 eventos por proyecto)
  const eventArrays = await Promise.all(projects.map(async (p) => {
    try {
      const r = await fetch(
        `${baseUrl}/api/v4/projects/${p.id}/events?per_page=10`,
        { headers: { 'Authorization': `Bearer ${token}` } }
      );
  ...
  // Paso 4: merge, sort, slice, normalizar con el pipeline común
  const allEvents = eventArrays.flat();
  allEvents.sort((a, b) => new Date(b.created_at) - new Date(a.created_at));
  return normalizeGitLabEvents(allEvents.slice(0, 30), projectMap, baseUrl);
}
