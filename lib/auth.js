 ...
 */
export async function authenticateGitHub(isRetry = false) {
  ...
  // Verificar que el token tenga los scopes necesarios ('repo' para repos privados, 'read:org' para org repos).
  // Si GitHub cacheo una autorización antigua con scopes insuficientes, faltarán scopes.
  // En ese caso, revocar el grant y re-autenticar para forzar la pantalla de consentimiento.
  const scopes = await verifyGitHubScopes(token.access_token);
  const hasRequired = scopes.includes('repo') && scopes.includes('read:org');
  if (!hasRequired && !isRetry) {
    await revokeGitHubGrant(token.access_token);
    await chrome.storage.local.remove('github_token');
    return authenticateGitHub(true);
  }
...
/**
 * Verifica los scopes OAuth del token de GitHub haciendo una llamada a /user
 * y leyendo el header X-OAuth-Scopes de la respuesta.
 *
 * @param {string} token - Token de acceso de GitHub.
 * @returns {Promise<string[]>} Lista de scopes del token (ej: ['repo', 'read:user']).
 */
async function verifyGitHubScopes(token) {
  try {
...
/**
 * Revoca la autorización OAuth completa de la app en GitHub.
 * Esto fuerza que la próxima conexión muestre la pantalla de consentimiento.
 *
 * @param {string} token - Token de acceso a revocar.
 */
export async function revokeGitHubGrant(token) {
  try {
  ...
  } catch { /* Ignorar errores de revocación — el token local se limpiará de todos modos */ }
}
