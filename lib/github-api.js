...
export async function fetchGitHubActivity() {
  ...
  const url = `${GITHUB_REST_URL}/users/${encodeURIComponent(username)}/events?per_page=30`;
  console.log('[DEBUG GitHub Activity] username:', username);
  console.log('[DEBUG GitHub Activity] url:', url);
  console.log('[DEBUG GitHub Activity] token (first 10 chars):', token.substring(0, 10) + '...');

  const response = await fetch(url, {
    headers: { 'Authorization': `Bearer ${token}` },
  });

  console.log('[DEBUG GitHub Activity] response status:', response.status);
  console.log('[DEBUG GitHub Activity] response headers:', Object.fromEntries(response.headers.entries()));

  if (response.status === 401) throw new Error('UNAUTHORIZED');
  if (!response.ok) throw new Error(`GitHub events API error: ${response.status}`);

  const events = await response.json();
  console.log('[DEBUG GitHub Activity] events count:', events.length);
  console.log('[DEBUG GitHub Activity] raw events:', JSON.stringify(events).substring(0, 500));
