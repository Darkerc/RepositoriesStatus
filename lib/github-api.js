 ...
 */
async function fetchOrgPublicEvents(token, orgLogin) {
  try {
    const response = await fetch(
      `${GITHUB_REST_URL}/orgs/${encodeURIComponent(orgLogin)}/events?per_page=30`,
      { headers: { 'Authorization': `Bearer ${token}` } }
    );
    if (response.status === 401) throw new Error('UNAUTHORIZED');
    if (!response.ok) return [];
 ...
 * @returns {Promise<Array<Object>>} Eventos crudos (o [] en error no fatal).
 */
async function fetchRepoEvents(token, fullName) {
  try {
    const response = await fetch(
      `${GITHUB_REST_URL}/repos/${fullName}/events?per_page=10`,
      { headers: { 'Authorization': `Bearer ${token}` } }
    );
    if (response.status === 401) throw new Error('UNAUTHORIZED');
    if (!response.ok) return [];
 ...
 */
export async function fetchGitHubGroupActivity(orgLogin) {
  const token = await getToken('github');
  if (!token) throw new Error('Not authenticated with GitHub');
  const viewerLogin = await getGitHubUsername();

  // Paso 1: eventos públicos de la org + lista de repos accesibles en paralelo
  const [orgEvents, orgRepos] = await Promise.all([
    fetchOrgPublicEvents(token, orgLogin),
    fetchOrgTopRepos(token, orgLogin),
  ]);

  // Paso 2: por cada repo top, obtener sus eventos (incluye repos privados accesibles)
  const repoEventArrays = await Promise.all(
    orgRepos.map(r => fetchRepoEvents(token, r.full_name))
  );

  // Paso 3: merge + dedupe por event.id
  const seen = new Set();
  const merged = [];
  for (const ev of [...orgEvents, ...repoEventArrays.flat()]) {
    if (!ev?.id || seen.has(ev.id)) continue;
    seen.add(ev.id);
    merged.push(ev);
  }

  // Paso 4: ordenar por fecha descendente, limitar a 30 y normalizar sin filtrar por viewer
  merged.sort((a, b) => new Date(b.created_at) - new Date(a.created_at));
  const top30 = merged.slice(0, 30);
  return await normalizeRestEvents(top30, viewerLogin, token, { filterByViewer: false });
}
