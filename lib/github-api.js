/**
 * @fileoverview Módulo de comunicación con la API de GitHub.
 *
 * Encapsula todas las llamadas a la API de GitHub necesarias para la extensión:
 * - Obtener contribuciones del último año (API GraphQL).
 * - Obtener el feed de actividad reciente del usuario (API REST).
 * - Obtener detalles de commits individuales (API REST).
 *
 * Flujo de datos:
 *   Token almacenado -> llamada a API -> datos crudos -> normalización -> datos uniformes
 *
 * Los datos normalizados tienen un formato común con GitLab para que el popup
 * pueda mostrarlos de forma unificada sin importar el proveedor.
 *
 * Dependencias: auth.js (obtención de tokens).
 */

import { getToken } from './auth.js';

/** @const {string} URL del endpoint GraphQL de GitHub */
const GITHUB_GRAPHQL_URL = 'https://api.github.com/graphql';

/** @const {string} URL base de la API REST de GitHub */
const GITHUB_REST_URL = 'https://api.github.com';

/**
 * Query GraphQL para obtener el calendario de contribuciones del usuario autenticado.
 * Retorna todas las semanas del último año con la fecha y cantidad de contribuciones por día.
 * Usa 'viewer' que automáticamente se refiere al usuario del token.
 * @const {string}
 */
const CONTRIBUTIONS_QUERY = `
query {
  viewer {
    contributionsCollection {
      contributionCalendar {
        weeks {
          contributionDays {
            date
            contributionCount
          }
        }
      }
    }
  }
}`;

/**
 * Query GraphQL para obtener la actividad reciente del usuario autenticado.
 * Usa 'viewer' para tener acceso a repos privados (a diferencia del REST Events API
 * que con tokens de GitHub Apps solo ve repos donde la App está instalada).
 *
 * Obtiene: PRs, issues, reviews y commits agrupados por repositorio.
 * @const {string}
 */
const ACTIVITY_QUERY = `
query {
  viewer {
    login
    contributionsCollection {
      pullRequestContributions(first: 20, orderBy: {direction: DESC}) {
        nodes {
          occurredAt
          pullRequest {
            id
            title
            number
            state
            url
            body
            repository {
              nameWithOwner
              isPrivate
            }
          }
        }
      }
      issueContributions(first: 20, orderBy: {direction: DESC}) {
        nodes {
          occurredAt
          issue {
            id
            title
            number
            state
            url
            body
            repository {
              nameWithOwner
              isPrivate
            }
          }
        }
      }
      pullRequestReviewContributions(first: 10, orderBy: {direction: DESC}) {
        nodes {
          occurredAt
          pullRequestReview {
            id
            body
            url
            pullRequest {
              title
              number
              repository {
                nameWithOwner
                isPrivate
              }
            }
          }
        }
      }
    }
    repositories(first: 15, orderBy: {field: PUSHED_AT, direction: DESC}, affiliations: [OWNER, ORGANIZATION_MEMBER, COLLABORATOR]) {
      nodes {
        nameWithOwner
        isPrivate
        defaultBranchRef {
          name
          target {
            ... on Commit {
              history(first: 10) {
                nodes {
                  oid
                  message
                  committedDate
                  url
                  author {
                    user {
                      login
                    }
                  }
                }
              }
            }
          }
        }
      }
    }
  }
}`;

/**
 * Obtiene los datos de contribuciones de GitHub del último año.
 * Usa la API GraphQL de GitHub para obtener el calendario de contribuciones completo.
 *
 * Flujo: obtener token -> enviar query GraphQL -> extraer semanas -> aplanar a mapa fecha->conteo.
 *
 * @returns {Promise<Object.<string, number>>} Objeto normalizado { "YYYY-MM-DD": conteo }
 *   donde cada clave es una fecha y el valor es la cantidad de contribuciones ese día.
 * @throws {Error} 'Not authenticated with GitHub' si no hay token almacenado.
 * @throws {Error} 'UNAUTHORIZED' si el token ha expirado o es inválido (HTTP 401).
 * @throws {Error} 'RATE_LIMITED:<timestamp>' si se excedió el límite de peticiones (HTTP 403).
 * @throws {Error} Error de GraphQL si la query falla.
 */
export async function fetchGitHubContributions() {
  const token = await getToken('github');
  if (!token) throw new Error('Not authenticated with GitHub');

  // Enviar la query GraphQL con autenticación Bearer
  const response = await fetch(GITHUB_GRAPHQL_URL, {
    method: 'POST',
    headers: {
      'Authorization': `Bearer ${token}`,
      'Content-Type': 'application/json',
    },
    body: JSON.stringify({ query: CONTRIBUTIONS_QUERY }),
  });

  // HTTP 401: token inválido o expirado, el background.js lo manejará eliminando el token
  if (response.status === 401) throw new Error('UNAUTHORIZED');

  if (!response.ok) {
    // HTTP 403 con header x-ratelimit-reset indica rate limiting
    if (response.status === 403) {
      const resetHeader = response.headers.get('x-ratelimit-reset');
      if (resetHeader) throw new Error(`RATE_LIMITED:${resetHeader}`);
    }
    throw new Error(`GitHub API error: ${response.status}`);
  }

  const json = await response.json();

  // GraphQL puede retornar HTTP 200 con errores en el cuerpo de la respuesta
  if (json.errors) throw new Error(`GitHub GraphQL error: ${json.errors[0].message}`);

  // Extraer las semanas del calendario de contribuciones
  const weeks = json.data.viewer.contributionsCollection.contributionCalendar.weeks;

  // Aplanar la estructura de semanas -> días a un mapa simple { fecha: conteo }
  const contributions = {};
  for (const week of weeks) {
    for (const day of week.contributionDays) {
      contributions[day.date] = day.contributionCount;
    }
  }
  return contributions;
}

/**
 * Obtiene el nombre de usuario de GitHub del usuario autenticado.
 * El resultado se cachea en chrome.storage.local para evitar llamadas repetidas.
 *
 * @returns {Promise<string>} Nombre de usuario de GitHub (login).
 * @throws {Error} Si la llamada a la API falla.
 */
async function getGitHubUsername() {
  // Verificar si ya tenemos el username en caché
  const cached = await chrome.storage.local.get('github_username');
  if (cached.github_username) return cached.github_username;

  // Si no está en caché, hacer una llamada a la API REST /user
  const token = await getToken('github');
  const response = await fetch(`${GITHUB_REST_URL}/user`, {
    headers: { 'Authorization': `Bearer ${token}` },
  });
  if (!response.ok) throw new Error(`GitHub user API error: ${response.status}`);

  const user = await response.json();
  // Guardar el username en caché para futuras consultas
  await chrome.storage.local.set({ github_username: user.login });
  return user.login;
}

/**
 * Obtiene la actividad reciente del usuario de GitHub combinando dos fuentes:
 *
 * 1. **GraphQL** (ACTIVITY_QUERY): Detalle de PRs, issues, reviews y commits de repos propios.
 * 2. **REST Events API** (/users/{username}/events): Captura actividad de TODOS los repos
 *    incluyendo organizaciones y repos privados de org (que GraphQL contributionsCollection omite).
 *
 * Los resultados se mergean y deduplican por ID para evitar duplicados.
 *
 * @returns {Promise<Array<Object>>} Arreglo de elementos de actividad normalizados.
 *   Cada elemento tiene: id, provider, repo, date, type, title, y campos adicionales según el tipo.
 * @throws {Error} 'Not authenticated with GitHub' si no hay token.
 * @throws {Error} 'UNAUTHORIZED' si el token es inválido.
 */
export async function fetchGitHubActivity({ page = 1 } = {}) {
  const token = await getToken('github');
  if (!token) throw new Error('Not authenticated with GitHub');

  // Obtener username para la REST Events API
  const username = await getGitHubUsername();

  // GraphQL solo aporta en la primera página (contributionsCollection no se pagina aquí).
  // En páginas > 1 únicamente se piden más eventos REST.
  const restPromise = fetchRestEvents(token, username, page);
  const graphqlPromise = page === 1 ? fetchGraphQLActivity(token) : Promise.resolve({ items: [] });

  const [graphqlRes, restRes] = await Promise.all([graphqlPromise, restPromise]);
  const graphqlItems = graphqlRes.items || [];
  const restItems = restRes.items || [];

  // Mergear y deduplicar dentro de la página
  const seen = new Set();
  const merged = [];
  for (const item of graphqlItems) {
    seen.add(item.id);
    merged.push(item);
  }
  for (const item of restItems) {
    if (!seen.has(item.id)) {
      seen.add(item.id);
      merged.push(item);
    }
  }

  merged.sort((a, b) => new Date(b.date) - new Date(a.date));
  // hasMore se apoya en la respuesta REST (la fuente paginable).
  return { data: merged, hasMore: restRes.hasMore };
}

/**
 * Obtiene actividad via la API GraphQL de GitHub.
 * Fuente principal para PRs, issues y reviews (tienen más detalle que REST).
 *
 * @param {string} token - Token de acceso.
 * @returns {Promise<Array<Object>>} Items normalizados de GraphQL.
 */
async function fetchGraphQLActivity(token) {
  try {
    const response = await fetch(GITHUB_GRAPHQL_URL, {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${token}`,
        'Content-Type': 'application/json',
      },
      body: JSON.stringify({ query: ACTIVITY_QUERY }),
    });

    if (response.status === 401) throw new Error('UNAUTHORIZED');

    if (!response.ok) {
      if (response.status === 403) {
        const resetHeader = response.headers.get('x-ratelimit-reset');
        if (resetHeader) throw new Error(`RATE_LIMITED:${resetHeader}`);
      }
      throw new Error(`GitHub API error: ${response.status}`);
    }

    const json = await response.json();
    if (json.errors) throw new Error(`GitHub GraphQL error: ${json.errors[0].message}`);

    return { items: normalizeGraphQLActivity(json.data) };
  } catch (err) {
    // Si GraphQL falla pero no es UNAUTHORIZED, retornar vacío (REST puede compensar)
    if (err.message === 'UNAUTHORIZED') throw err;
    return { items: [] };
  }
}

/**
 * Obtiene eventos recientes del usuario via la REST Events API.
 * Esta API captura actividad de TODOS los repos accesibles, incluyendo repos privados
 * de organizaciones que GraphQL contributionsCollection no muestra.
 *
 * @param {string} token - Token de acceso.
 * @param {string} username - Login del usuario.
 * @returns {Promise<Array<Object>>} Items normalizados de REST events.
 */
async function fetchRestEvents(token, username, page = 1) {
  // Tamaño de página para el scroll infinito. GitHub limita la Events API a
  // ~300 eventos totales (10 páginas con per_page=30).
  const perPage = 30;
  try {
    const response = await fetch(
      `${GITHUB_REST_URL}/users/${username}/events?per_page=${perPage}&page=${page}`,
      { headers: { 'Authorization': `Bearer ${token}` } },
    );

    if (!response.ok) return { items: [], hasMore: false };

    const events = await response.json();
    if (!Array.isArray(events)) return { items: [], hasMore: false };

    const items = await normalizeRestEvents(events, username, token);
    // Si la API devolvió una página completa, asumimos que puede haber más.
    const hasMore = events.length >= perPage;
    return { items, hasMore };
  } catch {
    return { items: [], hasMore: false };
  }
}

/**
 * Normaliza eventos de la REST Events API al formato uniforme del popup.
 * Para PushEvents con payload.commits vacío (común en repos privados de org),
 * obtiene el detalle del commit via REST API usando payload.head.
 *
 * @param {Array<Object>} events - Eventos crudos de la REST API.
 * @param {string} viewerLogin - Login del usuario autenticado.
 * @param {string} token - Token de acceso para fetch de detalles de commits.
 * @param {Object} [options]
 * @param {boolean} [options.filterByViewer=true] - Si es true, descarta eventos cuyo actor no sea el usuario.
 *   Se pone en false para la vista de actividad de un grupo/org, donde se quieren ver los eventos
 *   de todos los miembros.
 * @returns {Promise<Array<Object>>} Eventos normalizados.
 */
async function normalizeRestEvents(events, viewerLogin, token, { filterByViewer = true } = {}) {
  const items = [];
  const enrichPromises = [];

  for (const event of events) {
    // En la vista de usuario, solo eventos del propio usuario.
    // En la vista de grupo, se incluyen los eventos de todos los miembros.
    if (filterByViewer && event.actor?.login !== viewerLogin) continue;

    const repo = event.repo?.name || '';
    const date = event.created_at;
    const actor = event.actor?.login || '';

    switch (event.type) {
      case 'PushEvent': {
        const commits = event.payload?.commits || [];
        const branch = (event.payload?.ref || '').replace('refs/heads/', '');

        if (commits.length > 0) {
          // Commits disponibles en el payload (repos públicos / propios)
          for (const commit of commits) {
            items.push({
              id: `gh-${commit.sha}`,
              provider: 'github',
              repo,
              date,
              actor,
              type: 'commit',
              title: commit.message.split('\n')[0],
              branch,
              author: commit.author?.name || '',
              commits: [{ sha: commit.sha, message: commit.message }],
            });
          }
        } else {
          // Commits vacíos (común en repos privados de org) — obtener detalle via API
          const headSha = event.payload?.head || '';
          if (headSha) {
            const idx = items.length;
            items.push({
              id: `gh-${headSha}`,
              provider: 'github',
              repo,
              date,
              actor,
              type: 'commit',
              title: headSha.substring(0, 7),
              branch,
              author: '',
              commits: [{ sha: headSha, message: '' }],
            });
            // Encolar fetch de detalle (se ejecutan en paralelo al final)
            enrichPromises.push(
              fetchCommitDetailLight(token, repo, headSha).then(detail => {
                if (detail) {
                  items[idx].title = detail.message;
                  items[idx].author = detail.author;
                  items[idx].commits[0].message = detail.fullMessage;
                }
              })
            );
          }
        }
        break;
      }
      case 'CreateEvent': {
        const refType = event.payload?.ref_type;
        if (refType === 'branch' || refType === 'tag') {
          items.push({
            id: `gh-create-${event.id}`,
            provider: 'github',
            repo,
            date,
            actor,
            type: refType,
            title: `${event.payload.ref || repo}`,
            action: `created ${refType}`,
          });
        }
        break;
      }
      case 'DeleteEvent': {
        const refType = event.payload?.ref_type;
        if (refType === 'branch' || refType === 'tag') {
          items.push({
            id: `gh-delete-${event.id}`,
            provider: 'github',
            repo,
            date,
            actor,
            type: refType,
            title: `${event.payload.ref || repo}`,
            action: `deleted ${refType}`,
          });
        }
        break;
      }
      case 'IssuesEvent': {
        const issue = event.payload?.issue;
        if (issue) {
          items.push({
            id: `gh-issue-${issue.id}`,
            provider: 'github',
            repo,
            date,
            actor,
            type: 'issue',
            title: issue.title,
            number: issue.number,
            state: issue.state,
            action: event.payload.action,
            url: issue.html_url,
          });
        }
        break;
      }
      case 'IssueCommentEvent': {
        const comment = event.payload?.comment;
        const issue = event.payload?.issue;
        if (comment && issue) {
          items.push({
            id: `gh-comment-${comment.id}`,
            provider: 'github',
            repo,
            date,
            actor,
            type: 'comment',
            title: issue.title,
            number: issue.number,
            description: (comment.body || '').substring(0, 300),
            url: comment.html_url,
          });
        }
        break;
      }
      case 'PullRequestEvent': {
        const pr = event.payload?.pull_request;
        if (pr) {
          items.push({
            id: `gh-pr-${pr.id}`,
            provider: 'github',
            repo,
            date,
            actor,
            type: 'pull_request',
            title: pr.title,
            number: pr.number,
            state: pr.state,
            action: event.payload.action,
            description: (pr.body || '').substring(0, 300),
            url: pr.html_url,
          });
        }
        break;
      }
      case 'PullRequestReviewEvent': {
        const review = event.payload?.review;
        const pr = event.payload?.pull_request;
        if (review && pr) {
          items.push({
            id: `gh-review-${review.id}`,
            provider: 'github',
            repo,
            date,
            actor,
            type: 'review',
            title: `Reviewed PR #${pr.number}: ${pr.title}`,
            description: (review.body || '').substring(0, 300),
            url: review.html_url,
          });
        }
        break;
      }
      case 'ForkEvent':
        items.push({
          id: `gh-fork-${event.id}`,
          provider: 'github',
          repo,
          date,
          actor,
          type: 'fork',
          title: event.payload?.forkee?.full_name || repo,
          action: 'forked',
        });
        break;
      case 'WatchEvent':
        items.push({
          id: `gh-star-${event.id}`,
          provider: 'github',
          repo,
          date,
          actor,
          type: 'star',
          title: repo,
          action: 'starred',
        });
        break;
    }
  }

  // Enriquecer commits con detalle (máx 10 fetches en paralelo para evitar rate limiting)
  if (enrichPromises.length > 0) {
    await Promise.allSettled(enrichPromises.slice(0, 10));
  }

  return items;
}

/**
 * Obtiene info básica de un commit (mensaje y autor) via REST API.
 * Se usa como fallback cuando el PushEvent no trae commits en el payload.
 *
 * @param {string} token - Token de acceso.
 * @param {string} repo - Nombre del repositorio (owner/repo).
 * @param {string} sha - Hash SHA del commit.
 * @returns {Promise<Object|null>} { message, fullMessage, author } o null si falla.
 */
async function fetchCommitDetailLight(token, repo, sha) {
  try {
    const response = await fetch(`${GITHUB_REST_URL}/repos/${repo}/commits/${sha}`, {
      headers: { 'Authorization': `Bearer ${token}` },
    });
    if (!response.ok) return null;
    const data = await response.json();
    return {
      message: data.commit.message.split('\n')[0],
      fullMessage: data.commit.message,
      author: data.commit.author.name,
    };
  } catch {
    return null;
  }
}

/**
 * Normaliza la respuesta de la query GraphQL de actividad a un formato uniforme.
 *
 * Transforma las contribuciones de GraphQL (PRs, issues, reviews, commits por repo)
 * a un formato común con campos estándar, compatible con los datos de GitLab.
 *
 * @param {Object} data - Datos del campo 'data' de la respuesta GraphQL.
 * @returns {Array<Object>} Eventos normalizados, ordenados por fecha descendente, máximo 30.
 */
function normalizeGraphQLActivity(data) {
  const items = [];
  const cc = data.viewer.contributionsCollection;

  // Pull Requests
  for (const node of cc.pullRequestContributions.nodes) {
    const pr = node.pullRequest;
    items.push({
      id: `gh-pr-${pr.id}`,
      provider: 'github',
      repo: pr.repository.nameWithOwner,
      isPrivate: pr.repository.isPrivate,
      date: node.occurredAt,
      type: 'pull_request',
      title: pr.title,
      number: pr.number,
      state: pr.state.toLowerCase(),
      description: pr.body?.substring(0, 300) || '',
      url: pr.url,
    });
  }

  // Issues
  for (const node of cc.issueContributions.nodes) {
    const issue = node.issue;
    items.push({
      id: `gh-issue-${issue.id}`,
      provider: 'github',
      repo: issue.repository.nameWithOwner,
      isPrivate: issue.repository.isPrivate,
      date: node.occurredAt,
      type: 'issue',
      title: issue.title,
      number: issue.number,
      state: issue.state.toLowerCase(),
      description: issue.body?.substring(0, 300) || '',
      url: issue.url,
    });
  }

  // Reviews de PRs
  for (const node of cc.pullRequestReviewContributions.nodes) {
    const review = node.pullRequestReview;
    const pr = review.pullRequest;
    items.push({
      id: `gh-review-${review.id}`,
      provider: 'github',
      repo: pr.repository.nameWithOwner,
      isPrivate: pr.repository.isPrivate,
      date: node.occurredAt,
      type: 'review',
      title: `Reviewed PR #${pr.number}: ${pr.title}`,
      description: review.body?.substring(0, 300) || '',
      url: review.url,
    });
  }

  // Commits individuales de los repos más recientes del usuario
  const viewerLogin = data.viewer.login;
  for (const repo of (data.viewer.repositories?.nodes || [])) {
    const repoName = repo.nameWithOwner;
    const branchName = repo.defaultBranchRef?.name || '';
    const commits = repo.defaultBranchRef?.target?.history?.nodes || [];
    for (const commit of commits) {
      // Solo incluir commits del usuario autenticado
      if (commit.author?.user?.login !== viewerLogin) continue;
      items.push({
        id: `gh-${commit.oid}`,
        provider: 'github',
        repo: repoName,
        isPrivate: repo.isPrivate,
        date: commit.committedDate,
        type: 'commit',
        title: commit.message.split('\n')[0],
        branch: branchName,
        author: commit.author?.user?.login || '',
        commits: [{
          sha: commit.oid,
          message: commit.message,
        }],
      });
    }
  }

  // Ordenar por fecha descendente (más reciente primero) y limitar a 30
  items.sort((a, b) => new Date(b.date) - new Date(a.date));
  return items.slice(0, 30);
}

/**
 * Obtiene los detalles de un commit específico de GitHub, incluyendo los archivos modificados.
 *
 * Se usa cuando el usuario hace clic en un commit en el feed de actividad
 * para ver qué archivos fueron cambiados y las estadísticas de líneas.
 *
 * @param {string} repo - Nombre del repositorio en formato 'owner/repo'.
 * @param {string} sha - Hash SHA del commit.
 * @returns {Promise<Object>} Detalles del commit con la estructura:
 *   { sha, message, author, date, url, stats: {additions, deletions, total},
 *     files: [{filename, status, additions, deletions}] }
 * @throws {Error} 'Not authenticated with GitHub' si no hay token.
 * @throws {Error} Error de API si el commit no existe o no es accesible.
 */
export async function fetchGitHubCommitDetail(repo, sha) {
  const token = await getToken('github');
  if (!token) throw new Error('Not authenticated with GitHub');

  // Llamar a la API REST para obtener los detalles completos del commit
  const response = await fetch(`${GITHUB_REST_URL}/repos/${repo}/commits/${sha}`, {
    headers: { 'Authorization': `Bearer ${token}` },
  });

  if (!response.ok) throw new Error(`GitHub commit API error: ${response.status}`);

  const commit = await response.json();

  // Normalizar la respuesta a nuestro formato estándar de detalle de commit
  return {
    sha: commit.sha,
    message: commit.commit.message,
    author: commit.commit.author.name,
    date: commit.commit.author.date,
    url: commit.html_url,
    stats: commit.stats,  // { additions, deletions, total }
    // Mapear los archivos a un formato simplificado
    files: (commit.files || []).map(f => ({
      filename: f.filename,
      status: f.status,       // 'added', 'removed', 'modified', 'renamed'
      additions: f.additions,
      deletions: f.deletions,
    })),
  };
}

// ── Lenguajes más usados ──

/**
 * Query GraphQL para obtener los lenguajes de los repos en los que el usuario
 * realmente ha hecho commits durante el último año (la misma ventana que usa
 * el heatmap).
 *
 * Por qué `contributionsCollection.commitContributionsByRepository` y no
 * `viewer.repositories(...affiliations:[ORGANIZATION_MEMBER,...])`:
 * - `viewer.repositories` con afiliación `ORGANIZATION_MEMBER` puede no listar
 *   repos privados de una org donde el usuario es miembro pero no colaborador
 *   directo (caso típico: SAML/SSO o repos donde el acceso es indirecto).
 * - `commitContributionsByRepository` devuelve los repos donde el usuario
 *   commiteó en el período de contribuciones (la misma fuente que alimenta
 *   el calendar del heatmap), incluyendo repos privados de orgs.
 *
 * Como complemento se incluyen los repos del propio viewer ordenados por push
 * para no perder repos donde el usuario sea owner aún sin commits recientes.
 *
 * `maxRepositories: 100` es el límite de la API. Después se recortan a 30 los
 * más relevantes (los que más bytes aportan).
 * @const {string}
 */
const LANGUAGES_QUERY = `
query {
  viewer {
    login
    contributionsCollection {
      commitContributionsByRepository(maxRepositories: 100) {
        contributions { totalCount }
        repository {
          nameWithOwner
          isPrivate
          languages(first: 10, orderBy: {field: SIZE, direction: DESC}) {
            edges {
              size
              node { name color }
            }
          }
        }
      }
    }
    repositories(first: 30, orderBy: {field: PUSHED_AT, direction: DESC}, affiliations: [OWNER, ORGANIZATION_MEMBER, COLLABORATOR], isFork: false) {
      nodes {
        nameWithOwner
        isPrivate
        languages(first: 10, orderBy: {field: SIZE, direction: DESC}) {
          edges {
            size
            node { name color }
          }
        }
      }
    }
  }
}`;

/**
 * Obtiene un mapa agregado de lenguajes más usados del usuario en GitHub.
 *
 * Estrategia (alineada con la fuente del heatmap):
 * 1. Pide en una sola query los repos donde el usuario commiteó el último año
 *    (`commitContributionsByRepository`) — incluye privados de orgs.
 * 2. Une con los repos owner/ORGANIZATION_MEMBER ordenados por push, por si
 *    hay repos relevantes sin commits recientes.
 * 3. Deduplica por `nameWithOwner` y se queda con los 30 con mayor `totalSize`
 *    (suma de bytes por lenguaje del repo).
 * 4. Suma bytes por nombre de lenguaje preservando el color oficial.
 *
 * @returns {Promise<{languages: Object.<string, {bytes: number, color: string|null}>, repoCount: number}>}
 *   - languages: mapa { nombre: { bytes, color } } con totales agregados.
 *   - repoCount: número de repos analizados (≤ 30).
 * @throws {Error} 'Not authenticated with GitHub' si no hay token.
 * @throws {Error} 'UNAUTHORIZED' si el token es inválido (HTTP 401).
 * @throws {Error} 'RATE_LIMITED:<timestamp>' si se excedió el límite.
 */
export async function fetchGitHubLanguages() {
  const token = await getToken('github');
  if (!token) throw new Error('Not authenticated with GitHub');

  const response = await fetch(GITHUB_GRAPHQL_URL, {
    method: 'POST',
    headers: {
      'Authorization': `Bearer ${token}`,
      'Content-Type': 'application/json',
    },
    body: JSON.stringify({ query: LANGUAGES_QUERY }),
  });

  if (response.status === 401) throw new Error('UNAUTHORIZED');
  if (!response.ok) {
    if (response.status === 403) {
      const resetHeader = response.headers.get('x-ratelimit-reset');
      if (resetHeader) throw new Error(`RATE_LIMITED:${resetHeader}`);
    }
    throw new Error(`GitHub API error: ${response.status}`);
  }

  const json = await response.json();
  if (json.errors) throw new Error(`GitHub GraphQL error: ${json.errors[0].message}`);

  const data = json.data?.viewer || {};

  // Recopilar repos de las dos fuentes, deduplicando por nameWithOwner.
  // Cada entrada conserva sus edges de lenguajes y un score (bytes totales) para
  // luego priorizar los repos con más código a la hora de recortar a 30.
  const repoMap = new Map();
  const ingestRepo = (repo) => {
    if (!repo?.nameWithOwner) return;
    if (repoMap.has(repo.nameWithOwner)) return; // primera fuente gana
    const edges = repo.languages?.edges || [];
    let totalBytes = 0;
    for (const e of edges) totalBytes += e.size || 0;
    repoMap.set(repo.nameWithOwner, { edges, totalBytes });
  };

  // Fuente principal: repos donde el usuario realmente commiteó.
  const commitRepos = data.contributionsCollection?.commitContributionsByRepository || [];
  for (const entry of commitRepos) ingestRepo(entry?.repository);

  // Complemento: repos owner / org-member más recientemente pusheados.
  const ownedRepos = data.repositories?.nodes || [];
  for (const repo of ownedRepos) ingestRepo(repo);

  // Top 30 por tamaño total (proxy razonable de "repos donde más código hay").
  const topRepos = [...repoMap.values()]
    .sort((a, b) => b.totalBytes - a.totalBytes)
    .slice(0, 30);

  // Agregar bytes por lenguaje, preservando el color oficial.
  const languages = {};
  for (const repo of topRepos) {
    for (const edge of repo.edges) {
      const name = edge.node?.name;
      if (!name) continue;
      if (!languages[name]) {
        languages[name] = { bytes: 0, color: edge.node.color || null };
      }
      languages[name].bytes += edge.size || 0;
      if (!languages[name].color && edge.node.color) {
        languages[name].color = edge.node.color;
      }
    }
  }

  return { languages, repoCount: topRepos.length };
}

// ── Actividad de organizaciones (grupos en GitHub) ──

/**
 * Obtiene las organizaciones a las que pertenece el usuario autenticado.
 * Se usa para poblar la lista de sub-tabs de la vista "Actividad de grupos".
 *
 * Requiere scope 'read:org' (ya solicitado en el flujo OAuth).
 *
 * @returns {Promise<Array<{login: string, name: string, avatarUrl: string, url: string}>>}
 * @throws {Error} 'Not authenticated with GitHub' si no hay token.
 * @throws {Error} 'UNAUTHORIZED' si el token es inválido (HTTP 401).
 * @throws {Error} 'RATE_LIMITED:<timestamp>' si se excedió el límite de peticiones.
 */
export async function fetchGitHubUserOrgs() {
  const token = await getToken('github');
  if (!token) throw new Error('Not authenticated with GitHub');

  const response = await fetch(`${GITHUB_REST_URL}/user/orgs?per_page=100`, {
    headers: { 'Authorization': `Bearer ${token}` },
  });

  if (response.status === 401) throw new Error('UNAUTHORIZED');
  if (!response.ok) {
    if (response.status === 403) {
      const resetHeader = response.headers.get('x-ratelimit-reset');
      if (resetHeader) throw new Error(`RATE_LIMITED:${resetHeader}`);
    }
    throw new Error(`GitHub orgs API error: ${response.status}`);
  }

  const orgs = await response.json();
  if (!Array.isArray(orgs)) return [];

  return orgs.map(o => ({
    login: o.login,
    name: o.login,      // /user/orgs no retorna display name; usar login como fallback
    avatarUrl: o.avatar_url || '',
    url: o.url || '',
  }));
}

/**
 * Fetch interno: eventos públicos de una organización.
 * 401 se re-lanza para que el caller maneje el flujo de sesión expirada.
 *
 * @param {string} token - Token de acceso.
 * @param {string} orgLogin - Nombre (login) de la organización.
 * @returns {Promise<Array<Object>>} Eventos crudos (o [] en error no fatal).
 */
async function fetchOrgPublicEvents(token, orgLogin, page = 1) {
  const perPage = 30;
  try {
    const response = await fetch(
      `${GITHUB_REST_URL}/orgs/${encodeURIComponent(orgLogin)}/events?per_page=${perPage}&page=${page}`,
      { headers: { 'Authorization': `Bearer ${token}` }, cache: 'no-store' }
    );
    if (response.status === 401) throw new Error('UNAUTHORIZED');
    if (!response.ok) return { events: [], hasMore: false };
    const events = await response.json();
    const arr = Array.isArray(events) ? events : [];
    return { events: arr, hasMore: arr.length >= perPage };
  } catch (err) {
    if (err.message === 'UNAUTHORIZED') throw err;
    return { events: [], hasMore: false };
  }
}

/**
 * Fetch interno: lista los repos de una org ordenados por último push.
 * Incluye repos privados donde el token tenga acceso (gracias al scope 'repo').
 *
 * @param {string} token - Token de acceso.
 * @param {string} orgLogin - Nombre (login) de la organización.
 * @returns {Promise<Array<{full_name: string}>>} Hasta 8 repos más recientes.
 */
async function fetchOrgTopRepos(token, orgLogin) {
  try {
    const response = await fetch(
      `${GITHUB_REST_URL}/orgs/${encodeURIComponent(orgLogin)}/repos?type=all&sort=pushed&per_page=8`,
      { headers: { 'Authorization': `Bearer ${token}` }, cache: 'no-store' }
    );
    if (response.status === 401) throw new Error('UNAUTHORIZED');
    if (!response.ok) return [];
    const repos = await response.json();
    if (!Array.isArray(repos)) return [];
    return repos.map(r => ({ full_name: r.full_name }));
  } catch (err) {
    if (err.message === 'UNAUTHORIZED') throw err;
    return [];
  }
}

/**
 * Fetch interno: eventos recientes de un repo específico.
 * Incluye actividad de otros miembros en repos privados donde el usuario tiene acceso.
 *
 * @param {string} token - Token de acceso.
 * @param {string} fullName - Nombre completo del repo ('owner/repo').
 * @returns {Promise<Array<Object>>} Eventos crudos (o [] en error no fatal).
 */
async function fetchRepoEvents(token, fullName, page = 1) {
  const perPage = 10;
  try {
    const response = await fetch(
      `${GITHUB_REST_URL}/repos/${fullName}/events?per_page=${perPage}&page=${page}`,
      { headers: { 'Authorization': `Bearer ${token}` }, cache: 'no-store' }
    );
    if (response.status === 401) throw new Error('UNAUTHORIZED');
    if (!response.ok) return { events: [], hasMore: false };
    const events = await response.json();
    const arr = Array.isArray(events) ? events : [];
    return { events: arr, hasMore: arr.length >= perPage };
  } catch (err) {
    if (err.message === 'UNAUTHORIZED') throw err;
    return { events: [], hasMore: false };
  }
}

/**
 * Obtiene la actividad reciente de una organización, combinando:
 * 1. Eventos públicos de la organización (/orgs/{org}/events).
 * 2. Eventos de los repos más activos de la org (incluidos privados accesibles).
 *
 * El merge + deduplicación por event.id evita entradas repetidas, y el resultado
 * se normaliza con el mismo pipeline que la actividad del usuario pero sin filtrar
 * por el viewer, de modo que se muestren acciones de todos los miembros.
 *
 * @param {string} orgLogin - Nombre (login) de la organización.
 * @returns {Promise<Array<Object>>} Hasta 30 items normalizados, ordenados por fecha desc.
 * @throws {Error} 'Not authenticated with GitHub' si no hay token.
 * @throws {Error} 'UNAUTHORIZED' si algún endpoint reporta 401.
 */
export async function fetchGitHubGroupActivity(orgLogin, { page = 1 } = {}) {
  const token = await getToken('github');
  if (!token) throw new Error('Not authenticated with GitHub');
  const viewerLogin = await getGitHubUsername();

  // Estrategia: la lista de repos top (hasta 8) se resuelve siempre desde la página 1
  // (ordenados por último push), pero los eventos de cada repo y los eventos
  // públicos de la org se paginan con el cursor `page`. Así, incluso si la org
  // tiene pocos repos, cada scroll extra trae eventos más antiguos de esos mismos
  // repos, en vez de agotarse enseguida.
  const orgEventsPromise = fetchOrgPublicEvents(token, orgLogin, page);
  const orgReposPromise = fetchOrgTopRepos(token, orgLogin);
  const [orgEventsRes, orgRepos] = await Promise.all([orgEventsPromise, orgReposPromise]);

  // Por cada repo top, obtener sus eventos en la página solicitada.
  const repoEventResults = orgRepos.length > 0
    ? await Promise.all(orgRepos.map(r => fetchRepoEvents(token, r.full_name, page)))
    : [];

  // Merge + dedupe por event.id
  const seen = new Set();
  const merged = [];
  const allEvents = [...orgEventsRes.events, ...repoEventResults.flatMap(r => r.events)];
  for (const ev of allEvents) {
    if (!ev?.id || seen.has(ev.id)) continue;
    seen.add(ev.id);
    merged.push(ev);
  }

  // Ordenar por fecha descendente y normalizar sin filtrar por viewer.
  merged.sort((a, b) => new Date(b.created_at) - new Date(a.created_at));
  const items = await normalizeRestEvents(merged, viewerLogin, token, { filterByViewer: false });

  // hasMore: mientras cualquiera de las fuentes (eventos de la org o de algún
  // repo) tenga una página completa, se asume que puede haber más. La Events API
  // de GitHub tiene un tope duro de ~10 páginas (300 eventos) por endpoint.
  const hasMore = orgEventsRes.hasMore || repoEventResults.some(r => r.hasMore);
  return { data: items, hasMore };
}
